...
def check_login(user, pwd):
    ...
    # Mencocokkan kolom 'id' dan 'pass' dari Sheet
    ...
    with tab1:
        ...
        if st.button("Masuk"):
            if check_login(u, p):
                ...
                st.rerun()
            else: st.error("User/Pass salah!")
    with tab2:
        nu = st.text_input("Username Baru", key="u2")
        np = st.text_input("Password Baru", type="password", key="p2")
        if st.button("Daftar"):
            client.open_by_key(SPREADSHEET_ID).worksheet('users').append_row([nu, np, 'user'])
            st.success("Berhasil! Silakan masuk.")
    st.stop()

# --- DASHBOARD ---
st.title("Monitoring Genset")
